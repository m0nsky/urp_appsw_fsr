...
namespace UnityEngine.Rendering.Universal
{
    ...
    public static class RenderingUtils
    {
        ...
        //internal static readonly int UNITY_STEREO_MATRIX_PREV_VP_L = Shader.PropertyToID("unity_MatrixPrevVP_L");
        //internal static readonly int UNITY_STEREO_MATRIX_PREV_VP_R = Shader.PropertyToID("unity_MatrixPrevVP_R");
        ...
        internal static void SetStereoViewAndProjectionMatrices(CommandBuffer cmd, Matrix4x4[] viewMatrix, Matrix4x4[] projMatrix, Matrix4x4[] cameraProjMatrix, bool setInverseMatrices, bool prevViewValid, Matrix4x4[] prevViewMatrix, bool isMotionVec = false)
        {
            if (frameIndex != Time.frameCount && isMotionVec)
            {
                //stereoConstants.mvViewProjMatrix = stereoConstants.prevMVViewProjMatrix;
                stereoConstants.mvViewProjMatrix.CopyTo(stereoConstants.prevMVViewProjMatrix, 0);
                for (int i = 0; i < 4; i++)
                {
                    for (int j = 0; j < 4; j++)
                    {
                        //stereoConstants.prevMVViewProjMatrix[i][j] += 0.2f;
                    }
                }
            ...
            //cmd.SetGlobalMatrix(UNITY_STEREO_MATRIX_PREV_VP_L, stereoConstants.prevMVViewProjMatrix[0]);
            //cmd.SetGlobalMatrix(UNITY_STEREO_MATRIX_PREV_VP_R, stereoConstants.prevMVViewProjMatrix[1]);
